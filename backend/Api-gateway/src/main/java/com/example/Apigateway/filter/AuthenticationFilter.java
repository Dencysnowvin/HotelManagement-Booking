...
public class AuthenticationFilter extends AbstractGatewayFilterFactory<AuthenticationFilter.Config> {
	...
	private boolean isAuthorized(String role, String path, String method) {
	    if ("admin".equalsIgnoreCase(role)) {
	        return path.startsWith("/api/bookings") || path.startsWith("/api/payment") || path.startsWith("/api/rooms");
	    } else if ("user".equalsIgnoreCase(role)) {
	        if (path.startsWith("/api/rooms") && method.equalsIgnoreCase("GET")) {
	            return true;
	        } else if ((path.startsWith("/api/bookings") || path.startsWith("/api/payment")) &&
	                   (method.equalsIgnoreCase("GET") || method.equalsIgnoreCase("POST") || method.equalsIgnoreCase("DELETE"))) {
	            return true;
	        }
	    }
	    return false;
